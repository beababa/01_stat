...
```{r, eval=FALSE}
data <- read.csv2("data/gros/rpls2021_geolocalise_OD_REG11_DEP93.csv",  fileEncoding = "ASCII")
#116 644
head(data)
table(data$LIBCOM)
...
Pour tous les fichiers, on sort le nombre de logements sociaux et la population
...
```{r}
read.csv("data/tabEE.csv", fileEncoding = "UTF-8")
```
...
```{r, eval = FALSE}
fic <- list.files("data/gros/", "REG11")
choix <- extrIDF$COM
choix
extrIDF
fic
...
# enregistrement jointure car fichier communes trop gros
st_write(jointure, "data/examen.gpkg","jointureFr", delete_layer = T)
```


```{r,  eval=FALSE}
jointure <- st_read("data/examen.gpkg", "jointureFr")
jointure [, c("nom", "insee")]
jointure <- jointure [-c(5:7,9:12),]
jointureDOM <- jointure [c(3,5),]
jointure<- jointure [-c(3,5),]
regionfr <- region [-c(1,3,8,12,14),]
png("img/fr.png")
mf_init(regionfr)
mf_map(region, col = "antiquewhite1", border = "antiquewhite3", add = T)
mf_map(jointure, col = "red", border = NA, add = T)
mf_label(jointure, var="nom", cex = 0.8, pos = 2 )
...
Une correction type est disponible sur le moodle. Les remarques qui suivent sont issues des copies dՎtudiants.
...
![](img/examComparaison.jpg)

### Qu'est ce que le RPLS ?
