...
```{r}
joint$NOM_COM_M[grep("ROCHE|ISSY|VITRY|ACH|POIN|CHENN", joint$NOM_COM_M)]
```
...
```{r}
st_write(bv, "data/gros/geo.gpkg", "bvValide", delete_layer = T)
```
...
579 communes c'est normale il y a les limitrophes.

Pas de filosofi à la Guadeloupe - moindre qualité des sources.


on va prendre les limites définis par les bv


```{r}

bvLimite <- aggregate(bv, by = list(bv$insee), st_union)
...
write.csv(fin,"data/devoirMaison/base.csv", fileEncoding = "UTF-8", row.names = FALSE, quote = T)
