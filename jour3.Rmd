...
Afin de vraiment comprendre, on saisit des modalités différentes :
- studio, T1 à T7
- sous sol premier deuxième...etc

Attention, valeur plancher 5

Travail sur la donnée : zonage par résidence, jointure

```{r}
library(sf)
st_layers("data/logement.gpkg")
zone <- st_read("data/logement.gpkg", "quartiersBailleurs")
rpls2154 <- st_read("data/logement.gpkg", "rpls2154")
# spatialisation du rpls
rpls <- read.csv("data/rplsBondy.csv", fileEncoding = "UTF-8", 
                 dec=",",
                 stringsAsFactors = T)
rpls <- rpls  [-3959,]
data2154 <- st_as_sf(data, coords = (c("X", "Y")), epsg = 2154)
data2154
library(mapsf)
mf_map(zone)
mf_map(rpls, add = T)
mf_map(rpls2154, add = T)
rpls2154 <-  st_transform(rpls2154, epsg = 4326)
st_coordinates(rpls2154)
tab <- table(data$X, useNA = "always")
tail(tab)
```
...
Par modalité, le nombre d'individus correspondant

Trouver les modalités possibles pour les deux variables
...
- répartition des étages puis lorsque ramené à une base 100, répartition des pièces
dans les étages (on peut comparer)
