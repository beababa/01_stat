...
Attention, 5 doublons de prénoms (Ambre, Camille, Marie, Roxane et Sarah), j'ai rajouté l'initiale
du nom de famille.
...
row.names(tab) <- c("cours1", "cours2", "cours3", "cours4")
knitr::kable(tab)
barplot(tab, beside = T, main = "nombre d'étudiants par groupe et par cours", xlab = "groupe", ylab = "nb", col = terrain.colors(4) 
        , border = NA, legend =  T)
```
...
barplot(rendu, names = titre, main = "rendu devoirs (effectif 75 étudiants environ)")
```
...
```{r}
devoir <- read.csv("data/notes.csv", fileEncoding = "UTF-8", dec=",", na.strings = NA)
hist(devoir$Total...25., main = "Répartion du cumul des notes des 3 devoirs (max 25)", freq = F, border = NA)
densite <- density(devoir$Total...25. )
...
```{r}
hist(devoir$Total...25., main = "Répartion du cumul des notes des 3 devoirs (min 5, max 25)", freq = F, border = NA,xlim = c(5,25), ylim = c(0,0.05))
lines(densite, col = "red", lwd = 3)
```
...
```{r}
rpls2019 <- read.csv2("G:/Mon Drive/05_DATA/05_HABITAT/RPLS2019_detail_IDF_DEP_93.csv")
rpls2021<- read.csv2("G:/Mon Drive/05_DATA/05_HABITAT/RPLS2021_detail_IDF_DEP_93.csv")
```
