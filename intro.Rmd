...
```{r}
ville <- read.csv("data/cours2.csv")
ville <- ville [,3]
villeModif <- read.csv("data/cours3.csv")
...
table(df$nb)
df <- df [!(df$nb != 5),]
# liste des villes utilisées
library(sf)
fr <- st_read("data/gros/COMMUNE.shp")
# jointure ville code insee pour les résultats
data2022 <- read.csv2("data/gros/resultats-par-niveau-burvot-t1-france-entiere.csv", fileEncoding = "Latin1", header = T)
names(data2022)
str(data2022$Code.de.la.commune)
# transformation en données texte
data2022$code <- as.character(data2022$code)
data2022$nbCar <- nchar(data2022$Code.de.la.commune)
table(data2022$nbCar)
paste0(data2022$Code.du.département, data2022$Code.de.la.commune)
```
...
res$denombrement [is.na(res$denombrement )& !is.na(res$univarié)] <- 0
res$denombrement [ is.na(res$univarié)] <- NA
# graphique
par(mfrow= c(2,2))
for (i in (1:length(res))){
  barplot(table(res[,i]), main = names(res)[i])
}
```
