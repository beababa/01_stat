...
```{r}
rendu <- c(47,36, 29, 14,14,11,4)
titre <- c("distribution", "univarié", "khi2", "khi2bis", "regression", "boxplot", "variance")
par(mar = c(8,6,6,6))
...
st_write(rpls2019, "data/data.gpkg", "rpls2019", delete_layer = T, quiet = T)
st_write(rpls2021, "data/data.gpkg", "rpls2021", delete_layer = T, quiet = T)
```


```{r}
library(sf)
rpls2019 <- st_read("data/data.gpkg", "rpls2019",fileEncoding = "UTF-8", quiet = T)
rpls2021 <- st_read("data/data.gpkg", "rpls2021",fileEncoding = "UTF-8", quiet = T)
library(mapsf)
