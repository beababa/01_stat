...
nb <- max(as.integer(substring(fic, 6,6)))
for (i in 2:nb) {
...
row.names(tab) <- substring(fic,1,6)
knitr::kable(tab)
```
...
Référence au niveau du département (pour 10 M hbts)
...
# récupération du fichier final
df <- read.csv("data/fr.csv", fileEncoding = "UTF-8")
tab <- table(df$zone)
tab
hist(tab, xlab = "nb de communes par zone", ylab = "nb d'occurences")
# Nb de communes par zone. On observe de grandes disparités.
# On supprime les communes avec peu de logements sociaux
summary(df$nb)
hist(df$nb, breaks = c(0,7, 22, 322,85, 47000))
...
#jointure et carto
# pour pouvoir utiliser la donnée le nom ne suffit pas, il faut le COG
cog <- read.csv("data/communes-01012019.csv", encoding = "UTF-8")
str(cog)
jointure <- merge(df, cog, by.x = "LIBCOM", by.y =  "libelle")
str(jointure)
...
write.csv(res, "data/fr.csv", fileEncoding = "UTF-8")
```
...
On ouvre le fichier sous le tableur et on le met en forme
- suppression des NA
- quelques doublons (à analyser)


Dernière vérif

```{r}
rpls <- read.csv("data/frVerif.csv", fileEncoding = "UTF-8")
summary(rpls$nb)
tab <- table(rpls$zone)
barplot(tab, las = 2)
```
...
Livrables :
    ...
    - un .pdf expliquant la démarche avec les principaux résultats

Ces 3 fichiers avec le prénom de l’étudiant.
