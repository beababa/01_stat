---
title: "Introduction"
output:
  html_document:
    number_sections: yes
    toc: yes
editor_options:
  chunk_output_type: console
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(eval  = TRUE)
knitr::opts_chunk$set(echo  = TRUE)
knitr::opts_chunk$set(warning  = FALSE)
``` 


# Intitulé et déroulé du cours


*"L5GEABIM Analyses bivariées et multivariées"*




```{r, eval=TRUE}
data <- read.csv("data/deroule2026.csv", encoding = "UTF-8")
knitr::kable(data)
```


Comptes des villes et élections municipales : quels sont les liens ?


# La donnée


## Où la trouver ?


Data.gouv, portail externe voire portail cartographique

### Data.gouv

définir avec wikipedia, que retenir par rapport à l'IA ?

apprendre à chercher :

- les élections (que préciser ?)

- les finances des communes (mot clé *compte des communes*)


### Le portail externe

Trouver la source originale pour les comptes des communes et apprendre à utiliser cet
outil plutôt que le fichier de data.gouv

car : 23 millions d'observations au total, un fichier de 6 G°de données

![](img/source.png)



### Portail cartographique

Utiliser l'[outil](https://data.ofgl.fr/pages/cartographie/) proposé par l'OFGL

Quel est l'apport de la cartographie ?


## Extraire la donnée à partir du portail externe

Explication des différents filtres


### Choix de base : compte consolidé et année


- compte consolidé

![](img/choixDeBase.png)


- pour le premier cours, ne pas faire de choix d'agrégat

![](img/agregatListe.png)

Sur la notion d'agrégat, au minimum :

- Savoir distinguer investissement et fonctionnement

- Dans quelle catégorie classer les frais de personnel ?


### La commune : un choix géographique différent par étudiant

Chaque étudiant choisit une commune et indique son code INSEE

```{r}
lieu <- read.csv("data/insee.csv")
lieu <- lieu [!is.na(lieu)]
suppressMessages(library(sf))
communes <- st_read("data/gros/communes-1000m.geojson", quiet = T)
region <- st_read("data/regions-20140306-100m.shp", quiet = T)
region$code_insee <- as.integer(region$code_insee)
region <- region [region$code_insee > 10,] 
data <- communes [communes$code %in% lieu,]
```


5 communes problématiques

```{r}
lieu [!(lieu %in% data$code)]
lieu <- c(lieu,"94028", "94003", "06088", "94037", "92062", "94080" )
data <- communes [communes$code %in% lieu,]
```


Cartographie des choix étudiants


```{r}
library(mapsf)
mf_map(region, col = "wheat", border="cornsilk")
mf_map(st_buffer(data, 10000), col= "red", add = T, border = NA)
mf_layout("Choix des communes\n(tampons autour des communes)", "data.gouv et sources internes\nseptembre 2026")
```




Enregistrement de la donnée

```{r}
st_write(data, "data/cours.gpkg", "commune", append = F, quiet = T)
data <- st_drop_geometry(data)
write.csv(data, "data/communeEtudiant.csv", row.names = F)
```



### Choix pour le cours


Le cours sera basé sur la commune de Bondy (49 enregistrements pour tous les agrégats) et le département du 93 (1967 enregistrements)


![](img/sel93.png)

A noter, sélectionner la région permet de choisir plus facilement le département 93

```{r}
data <- read.csv2("data/agregatEnsemble.csv", dec=".")
# liste des agrégats
names(data)
# début du fichier
head(data)
# structure du fichier
str(data)
# combien d'enregistrement par ville ?
table(data$Code.Insee.2025.Commune)
# filtre sur l'agrégat et le montant par hbt)
head(data [ , c(21,28)])
# sauvegarde de la donnée
write.csv(data [,c(21,28)], "data/agregatBondy.csv", fileEncoding = "UTF-8")
# Les agrégats ont-ils tous la même importance ?
hist(data$Montant.en...par.habitant)
```






# Présentation rapide (vous êtes nombreux !)

## Qui sont les étudiants ?

- 84 étudiants inscrits sur 3 cours (54 présents environ)

Les questions posées sont sur le passé / présent / futur des étudiants, leur appétence en stats


```{r}
etudiant <- read.csv("data/etudiantModifie.csv", fileEncoding = "UTF-8")
png("img/quisontils.png", width = 10000, height = 10000, res=1000)
#par(mar = c(2,16,2,4))
par(mfrow = c(2,2))
for (i in c(4, 3,2,1)){
  (mar = c(2,30,2,4))
  barplot(table(etudiant [,i]), border = NA,
          horiz = T, las = 2, cex.names = 0.8, 
          main = names (etudiant) [i])
}
dev.off()
```



![](img/quisontils.png)


## Présence et rendu des devoirs : chiffres clés

### Effectif et cours

```{r}
presence <- read.csv("data/presence.csv")
knitr::kable(presence)
presence$numCours <-as.numeric(substring(presence$Cours,2,3))
par(mfrow = c(3,1))
barplot(presence$TD1~presence$numCours, col="chartreuse", beside = T, main = "Evolution de la présence groupe 1", xlab = "cours", ylab = "nb" 
        , border = NA, legend =  F)
barplot(presence$TD2~presence$numCours, col = "cyan", beside = T, main = "Evolution de la présence groupe 2", xlab = "cours", ylab = "nb" 
        , border = NA, legend =  F)
barplot(presence$TD3~presence$numCours, col="darkorange",beside = T, main = "Evolution de la présence groupe 3", xlab = "cours", ylab = "nb" 
        , border = NA, legend =  F)
```



### Les rendus d'exercices

Attention, les exercices sont à rendre 2 jours avant le cours (temps de la correction), 

- gr 3 : mercredi 20 h

- gr 1 et 2 : jeudi 20 h

Aucun retard toléré

```{r}
data <- read.csv("data/notesDM1_7.csv", na.strings = "-", fileEncoding = "UTF-8")
data <- data [, c(7:12)]
names(data)
data <- data [, c(5,1,4,2,3,6)]
titre <- c("1_distribution", "2_univarié", "3_regression", "4_contingence", "5_khi2", "6_variance")

png("img/rendu.png", width = 600, height = 600, res = 100)
par( mfrow = c(2,3))
tot <- length(data)
i <-1
for (i in c(1:tot)){
  tmp <- table(data [,i])
  barplot(tmp,  cex.names = 0.8, las = 1, 
          main = paste0(titre [i], " (", sum(tmp), " observations)"), border = NA, col = "wheat")
  }

dev.off()
```

![](img/rendu.png)

- Le critère externe : des points faciles ?

le nom du fichier

le temps du rendu (avant la limite)

- Augmentation des erreurs avec l'exercice 4

Attention à la consigne !




## Notes finales



```{r, eval=FALSE}
note <- read.csv("data/noteFinales.csv", fileEncoding = "UTF-8", na.strings = c("EN ATTENTE", "NA","-"), dec = ",")
summary(note)
png("img/noteFin.png")
hist(note$NOTE.FINALE, main = "Etalement note finale - 64 obs", freq = T, border = NA)
abline(v = mean(note$NOTE.FINALE, na.rm = T),  col = "red")
abline(v = median(note$NOTE.FINALE, na.rm=T), col = "blue")
dev.off()
```

![](img/noteFin.png)

# Ressources autre que la bibliographie officielle


Pour R, la base de tout est 

R et espace (https://archives.framabook.org/r-et-espace/index.html)


