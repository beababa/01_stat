---
title: "Surfaces AB - Amélie Carrière"
author: "B. Maranget"
date: "28/09/2020"
output: 
  html_document: 
...
```{r}
dpt <- st_read(paste0(chemin,"ign.gpkg"), "dpt", quiet = TRUE, stringsAsFactors = F)
...
data <- read.xlsx("../dataE/01_CULTURES/2019_Donnes_Surfaces_Dept_Carto.xlsx",1, stringsAsFactors = F )
```
...
```{r, eval=FALSE}
# assemblage
noms <- list.files(pattern = ".xls")
data <- NULL
i <- 1
for (i in 1:length(noms)){
  tmp <- read.xlsx(noms [i])
...
rpg <- data [,c("Code_cultu", "an", "Surf_adm" ), drop = FALSE]
st_write(rpg, "../dataS/chrysomele.gpkg", "rpgBase", row.names = FALSE,quiet = TRUE, delete_layer = TRUE)
```
