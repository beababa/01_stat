...
```{r}
villes <- read.csv("data/etudiantGeomatique2.csv", fileEncoding = "UTF-8")
villes <- unique(villes$code.INSEE)
# 24
# 8 communes pour le 33 + Bondy
villes <- c(villes,  93010,33325,33100,33382,  33473, 33458, 33047, 33441, 33416)
villes <- unique(villes)
table(substring(villes,1,2))
```

33 communes inclu Bondy 

+ pt vigilance Paris 4 arrondissements

Rajouter Plassac, Cars, Saint Chrystophyde de blaye;33325,33100,33382, 
et 33473, 33458, 33047, 33441, 33416

Donc, on attend 33 communes en tout
...
villeSel <- as.numeric((unique(bvSel$insee)))
pb <- setdiff(villes, villeSel)
pb <- paste0("^", pb, "|")
pb [5] <- "^75109"
pb <- paste(pb, collapse = "")
# 28, manque les 4 arrondissements et 78598 Soisy sous montmorency 
# pb des arrondissements parisiens
paris <- (bv [grep(pb, bv$bureau),])
table(substring(paris$bureau,1,5))
...
pour les communes 75056 paris et pour les bv NA pour 2 communes du 33... donc 30, mais c'est ok pour 34

```{r}
str(bvSel)
bvSel <- bvSel [,-4]
# étrange champs block_ids qui a l'air lié avec les coordonnées
