...
nomfic <- paste0(Sys.getenv("oneDrive"), "\\data sauve\\ign.gpkg")
nomfic
# PB dans le st_read le fichier se transforme
fr <-  st_read("ign.gpkg", "france", quiet = TRUE, stringsAsFactors = F)
dpt <- st_read("ign.gpkg", "dpt", quiet = TRUE, stringsAsFactors = F)
```

Assemblage des fichiers excels

un peu erratique.... NE PAS REJOUER, les fichiers sont dans data.gpkg
...
# traitement des zéros = valeur NA
str(data)
...
for (i in levels(data$an)) {
  ...
  for (j in levels(data$culture)) {
    sel <- data[data$an == i & data$culture == j, ]
    total <- sum(sel$surface)
    sel$rel <- (sel$surface / total) * 100
    par(mar = c(0, 0, 1.2, 0))
    #centrage sur l'ensemble de la France
    ghostLayer(fr$geom, bg = "white")
    choroLayer(
      ...
      legend.title.cex = 0.6,
      legend.values.cex = 0.5,
      legend.title.txt = "part du total de la surface (%)",
      legend.border = "gray96",
      legend.pos = "left",
      add = TRUE
    )
